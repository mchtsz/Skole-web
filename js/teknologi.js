...
async function getFile() {
    ...
    const response = await fetch('https://raw.githubusercontent.com/mchtsz/markdown/main/teknologiforst%C3%A5else/markdowns.txt') /// endre url til filens plassering
...
async function getMarkdowns() {
    ...
    for (var filename of markdowns) {
        const response = await fetch(`https://raw.githubusercontent.com/mchtsz/markdown/main/teknologiforst%C3%A5else/${filename}.md`) /// endre url til filens plassering men behold ${filename} delen
        const text = await response.text()
        responses.push({
            text: text,
            name: filename
